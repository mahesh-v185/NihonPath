const termsOfService = `
# Terms of Service

**Effective Date:** 04/03/2025 (March 4th, 2025)

Welcome to **NihonPath** (the "Website"). By accessing or using our Website, you automatically agree to comply with and be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, please do not use our Website.


## 1. General Use  
- **Eligibility:** You must be at least 13 years old to use this Website.  
- **Purpose:** Our Website is designed to provide educational materials for learning Japanese. Use of the Website must comply with all applicable laws and these Terms.  
**Prohibited Activities.** You agree not to:
  - Use the Website for illegal or unauthorized activities.  
  - Disrupt, damage, or interfere with the Website’s functionality.  

## 2. Content Disclaimer  
- All content on this Website is provided for **educational purposes only**.
- We do not guarantee the accuracy, completeness, or reliability of the materials.  
- The Website’s content is subject to change without notice.  


## 3. Intellectual Property  
- **Ownership:** All text, images, graphics, and materials on the Website belong to **NihonPath** unless otherwise stated.  
- **Restrictions:** You may not copy, distribute, modify, or reproduce our content without permission.  

## 4. Third-Party Links  
- Our Website may contain links to third-party websites.  
- We are not responsible for the content or security of external websites.  
- Visiting such websites is at your own risk.  


## 5. Disclaimer of Warranties  
- Our Website is provided **"as is"** and **"as available"** without warranties of any kind.  
- We do not guarantee that the Website will be error-free or uninterrupted.  
- We are not responsible for **any losses** resulting from your use of the Website.  


## 6. Limitation of Liability  
- **NihonPath** is not liable for damages, **direct or indirect**, arising from your use of the Website.  
- We are not responsible for **data loss, system failures, or security breaches** related to using third-party services like Google Analytics.  


## 7. Changes to These Terms  
We may update these Terms from time to time. Changes will be posted on this page with an updated **Effective Date**. Continued use of the Website after changes means that you automatically accept the revised Terms.  


## 8. Contact Information  
If you have any questions or concerns about these Terms, contact us at:  

📧 **Email:** lingdojo.dev@gmail.com  


**Thank you for using NihonPath!** 🚀  
`;

export default termsOfService;
